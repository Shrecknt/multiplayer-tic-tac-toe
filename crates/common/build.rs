...
fn main() {
    ...
    let mut codegen = String::from("fn read_packet_string<T: std::io::Read>(buf: &mut T) -> Result<String, Box<dyn std::error::Error>> {\n    use integer_encoding::VarIntReader;\n    let string_length: u32 = buf.read_varint()?;\n    let mut res = vec![0u8; string_length as usize];\n    buf.read_exact(&mut res)?;\n    Ok(std::str::from_utf8(&res)?.to_string())\n}\n\nfn write_packet_string<T: std::io::Write>(\n    buf: &mut T,\n    src: &str,\n) -> Result<(), Box<dyn std::error::Error>> {\n    use integer_encoding::VarIntWriter;\n    let string_bytes = src.as_bytes();\n    let string_length = string_bytes.len();\n    buf.write_varint(string_length)?;\n    buf.write_all(string_bytes)?;\n    Ok(())\n}\n\nfn prepend_length(buf: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {\n    use integer_encoding::VarIntWriter;\n    use std::io::Write;\n    let mut res: Vec<u8> = vec![];\n    res.write_varint(buf.len())?;\n    res.write_all(buf)?;\n    Ok(res)\n}\n\npub trait Packet {\n    fn packet_id(&self) -> u32;\n    fn deserialize_slice<T: AsRef<[u8]>>(buf: &mut T) -> Result<Self, Box<dyn std::error::Error>>\n    where\n        Self: Sized;\n    fn deserialize(stream: &mut std::net::TcpStream) -> Result<Self, Box<dyn std::error::Error>>\n    where\n        Self: Sized;\n    fn serialize(&self) -> Result<Vec<u8>, Box<dyn std::error::Error>>;\n}\n\n");
    ...
    for (stage, value) in packets_obj {
        ...
        for (direction, packets) in value.as_object().unwrap() {
            ...
            let mut enum_impl = format!("impl Packet for {enum_name} {{\n");
